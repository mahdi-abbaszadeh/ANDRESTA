...
void proc_4(void ***inpargs, void ***outargs){
	...
	P4_OUT0_TYPE* out0 = (P4_OUT0_TYPE*) outargs[0];

	P4_OUT1_TYPE* out1 = (P4_OUT1_TYPE*) outargs[1];
...
void proc_5(void ***inpargs, void ***outargs){
	...
	P5_INP0_TYPE* inp0 = (P5_INP0_TYPE*) inpargs[0];

	P5_OUT0_TYPE* out0 = (P5_OUT0_TYPE*) outargs[0];
...
void proc_args_init(){
	...
	for(int i = 0; i < P4_OUT0_NUM_OF_TOKEN; i++)
		proc_4_outarg_0[i] = &proc_4_out_0[i];

	for(int i = 0; i < P4_OUT1_NUM_OF_TOKEN; i++)
		proc_4_outarg_1[i] = &proc_4_out_1[i];

	for(int i = 0; i < P5_INP0_NUM_OF_TOKEN; i++)
		proc_5_inparg_0[i] = &proc_5_inp_0[i];

	for(int i = 0; i < P5_OUT0_NUM_OF_TOKEN; i++)
		proc_5_outarg_0[i] = &proc_5_out_0[i];
...
int main()
{
	...
	for(int i =0; i<5; i++){
		...
		struct Edge *edge = get_edge(4,i,1);
		send_data(edge,4,i);
		}
